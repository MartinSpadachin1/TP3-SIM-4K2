...
function App() {
  // Estado para datos de las tablas (para poder validar y enviar)
  ...
  // Pasamos setters a componentes hijos para que actualicen el estado
  // (O alternativamente usar contexto o props callbacks)

  // Validaciones:
  const validarProbabilidades = () => {
    // Suma total 100 en probabilidades de la tabla simple
    const sumaSimple = probabilidades.reduce((acc, curr) => acc + Number(curr.valor || 0), 0);
    if (sumaSimple !== 100) {
      setError("La suma de probabilidades en 'Probabilidad de Embocar' debe ser 100%");
      return false;
    }
    // Validar suma 100 en cada grupo de ProbabilidadSegundaBola
    for (const grupo of datosSegundaBola) {
      const sumaGrupo = grupo.resultados.reduce((acc, res) => acc + Number(res.valor || 0), 0);
      if (sumaGrupo !== 100) {
    ...
    <>
      ...
      <Container fluid style={{ paddingTop: "15vh", minHeight: "85vh", marginTop: "3vh", width: "100vw" }}>
        <Row className="justify-content-center">
          ...
          >
            <ProbabilidadSegundaBola
              datos={datosSegundaBola}
              setDatos={setDatosSegundaBola}
            />
          </Col>
          <Col
            md={3}
